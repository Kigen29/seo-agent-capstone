...
Read `docs/DESIGN.md` first. A reference image is a brief for *layout and hierarchy*, never for
type, colour or shape: those come from the design system, not from the image.

So when working from a screenshot of another product, take the information architecture (what cards
exist, what each shows, how the nav groups) and re-express it in Classical: serif type, warm paper,
gold accent, small radii, the existing `.card` and `Stat` components. Do not reproduce the source's
palette, font, or corner radii, and do not introduce a component the design system already has an
answer for.
