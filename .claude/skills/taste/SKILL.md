...
## PROJECT OVERRIDE (Rankwright)

This repository already has a design system. Read `docs/DESIGN.md` before generating anything, and
prefer it over every default in this skill.

Concretely: serif type (Cormorant Garamond headings at weight 400, Lora body), warm paper
backgrounds (`#f3f2f2`, never `#fff`), gold accent used sparingly, small radii (2/4/7/12px, nothing
is a pill), and existing components (`.card`, `Stat`, `PageHeader`, `EmptyState`, `Note`,
`SeverityBadge`) rather than new ones.

The generic modern-SaaS direction this skill would otherwise reach for — Inter, a violet primary,
large radii, gradient hero — is wrong here. When this skill and `docs/DESIGN.md` disagree,
`docs/DESIGN.md` wins.
