...
| `@seo/fixers` | 94 | The framework detector per stack, the head injector, and each fixer with a triggering and a clean fixture: the canonical origin rewrite (with the hostname-boundary guard), the robots AI-crawler unblock, the noindex strip, the `llms.txt` writer, and the `LocalBusiness` schema block. |
